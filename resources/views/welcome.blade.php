            ...
            @foreach ($places as $place)
                <div class="my-1 px-1 w-full md:w-1/2 lg:my-4 lg:px-4 lg:w-1/3">
                    <article class="overflow-hidden rounded-lg shadow-lg bg-white">
                        <a href="">
                            {{-- asset('storage/images/'.$place->image) or  --}}
                            <img src="{{$place->image}}" alt="" class="block h-auto w-full">
                        </a>
                        <header class="items-center justify-between leading-tight p-2 md:p-4">
                            <h1 class="text-base mb-3">
                                <a href="" class="no-underline hover:underline text-black">{{$place->name}}</a>
                            </h1>
                              <h4 class="text-xl">{{$place->adderss}}</h4>
                        </header>
